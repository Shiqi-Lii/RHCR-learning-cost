...
def _build_baseline_cmd(cfg: dict, run_dir: Path, k: int, seed: int):
    cmd = _build_base_cmd(cfg, run_dir, k, seed)
    cmd.extend(["--use_learned_cost", "false"])
    return cmd
...
def main():
    parser = argparse.ArgumentParser(description="Simple RHCR comparison runner: baseline vs learned-cost over agents x seeds.")
    parser.add_argument(
        "--runner_config",
    ...
    if cfg.get("learned_cost_python", ""):
        p = cfg["learned_cost_python"]
        cfg["learned_cost_python"] = str((repo_root / p).resolve()) if not str(p).startswith("/") and "/" in str(p) else p
    ...
    tasks = []
    pairs = [("baseline", _build_baseline_cmd), ("learned", _build_learned_cmd)]
    for mode, builder in pairs:
        for k in agents:
            for seed in seeds:
                run_dir = out_root / mode / f"agents_{k}" / f"seed_{seed}"
                run_dir.mkdir(parents=True, exist_ok=True)
                cmd = builder(cfg, run_dir, k, seed)
